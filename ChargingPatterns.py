...
df_taxi = pd.read_csv('new_NbofEvs_1.csv', index_col=0, parse_dates=True)
...
def scheduling(medallion,SOCmin,SOCmax,SOCmedian,deltaEnergy,CB,r_c,r_d):
    T_d=[]
    Energy_level = [CB]
    SOC = [1]
...
def optimalParameter(medallion, SOCmax, deltaEnergy, CB):
        ...
        for b in np.arange(SOCmedian_initial, SOCmedian_max, SOCmedian_inc):
            ...
            for d in np.arange(r_c_initial, r_c_min + r_c_inc, r_c_inc):
                ...
                for c in np.arange(r_d_initial, r_d_min + r_d_inc, r_d_inc):
                    ...
                    if works:
                        #SOC,output, works = scheduling(medallion,a, SOCmax, b, deltaEnergy, CB, d, c)
                        print(f"Algorithm works with SOCmin={a}, SOCmedian={b}, r_c={d},r_d={c}.")
                        found_optimal = True
                        break
                if found_optimal:
                    break
    ...
    if not found_optimal:
        ...
        print("EV usage not possible as drives are too long, petrol needs to be used.")
...
def createdischargeavail(medallion, T_dopt,a,b,c,d):
    ...
    # Define the desired date range for the index
    date_range = pd.date_range(start='2013-01-01 00:00:00', end='2013-01-31 23:45:00', freq='15T')
...
#a,b,c,d,SOCopt,T_dopt=optimalParameter(medallion,SOCmax,deltaEnergy,CB)
#df_one_medaill=createdischargeavail(medallion,T_dopt)
#print(df_one_medaill)
# Get a list of all the medallions from the DataFrame columns
medallion_list = df_taxi.columns
medallion_list = np.delete(medallion_list, -1)
# Create an empty DataFrame to store the results for all medallions
all_medaillon_df = pd.DataFrame()

# Iterate through each medallion and call the optimalParameter and createdischargeavail functions
for medallion in medallion_list:
    print(medallion)
    a, b, c, d, SOCopt, T_dopt = optimalParameter(medallion, SOCmax, deltaEnergy, CB)
    medaillon_df = createdischargeavail(medallion, T_dopt,a,b,c,d)

    # Combine the resulting DataFrames
    all_medaillon_df = pd.concat([all_medaillon_df, medaillon_df], axis=1)
...
scaled_medaillon_df = all_medaillon_df.mul(discharging_rates)

# Sum all rows except the last 4, since they contain the SOCmin, SOCmedian, r_c, and r_d values
summed_rows = scaled_medaillon_df.iloc[:-4].sum(axis=0).to_frame().transpose()

# Save the summed_rows DataFrame as an Excel file
summed_rows.to_excel("summed_rows_scaled.xlsx", index=False)