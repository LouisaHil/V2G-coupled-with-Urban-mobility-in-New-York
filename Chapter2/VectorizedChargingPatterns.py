...
## this is NYC demand over 24 hours replicated for the entire month
...
# Read the data from taxis ( e.g. month of june taxi) 
df_taxi = pd.read_csv('new_NbofEvs_6.csv', index_col=0, parse_dates=True)
# this is the dataset for private vehicles expanded over an entire year --> change if wanting to run over private vehicles 
#df_taxi= pd.read_csv('expanded_dataset.csv', index_col='time_interval')
df_taxi.index = pd.to_datetime(df_taxi.index)

### if running for expanded dataset this needs to be uncommented as well 
#df_taxi=df_taxi[df_taxi.index.month == month] 
#f_taxi = df_taxi.iloc[:-100]
## definitions
...
def createdischargeavail(medallion, T_dopt,a,b,c,d):
    ...
    date_range = pd.date_range(start='2013-07-01 00:00:00', end='2013-07-30 23:45:00', freq='15T')
...
all_medaillon_df.to_excel("7PrivateMedaillonlist.xlsx", index=False)
...
scaled_medaillon_df.to_excel("7Private_scaled.xlsx", index=False)
...
summed_rows.to_excel("7Private_summed_rows_scaled.xlsx", index=False)
